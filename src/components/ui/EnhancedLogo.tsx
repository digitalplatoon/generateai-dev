...
const EnhancedLogo = ({ size = 'md', showText = true }: EnhancedLogoProps) => {
  const sizeClasses = {
    sm: 'w-6 h-6',
    md: 'w-8 h-8',
    lg: 'w-12 h-12'
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      {showText && (
        <span className={`text-white font-bold ${textSizeClasses[size]} bg-gradient-to-r from-white to-teal bg-clip-text text-transparent`}>
          GenerateAI.dev
        </span>
      )}
    </div>
